...
**🆕 本分支特色功能**：
- **⏰ 消息模式定时任务** - 基于 Cron 表达式的自动化任务调度，支持多种执行模式
- **📱 抖音评论引流** - 智能评论任务管理，支持 AI 生成回复内容
- **📊 抖音数据统计** - 回复历史记录与数据分析
...
[English Documentation](README_EN.md) | [上游仓库](https://github.com/suyiiyii/AutoGLM-GUI)
...
基于消息模式的定时任务，支持灵活的 Cron 表达式调度：

#### 创建定时任务

1. **进入管理页面** - 点击左侧导航栏的「定时任务」图标
2. **新建任务** - 点击右上角「新建任务」按钮
3. **配置任务**：
   - **任务名称**：给任务起一个易记的名称
   - **设备**：选择要执行任务的设备
   - **消息内容**：描述要执行的任务（如："打开微信，给张三发送早安"）
   - **Cron 表达式**：设置执行时间（如：`0 8 * * *` 表示每天早上 8 点）
   - **执行模式**：选择经典模式、双模型协作或分层代理
4. **保存并启用** - 任务会按照设定的时间自动执行
...
- **经典模式**：单一视觉模型直接执行，适合简单任务
- **双模型协作**：决策模型 + 视觉模型协作
- **分层代理**：规划层 + 执行层分离，适合复杂任务

### 📱 抖音评论引流任务（本分支特色）

智能化的抖音评论互动管理，帮助进行内容营销：

#### 创建评论任务

1. **进入管理页面** - 点击左侧导航栏的「抖音评论」图标
2. **新建任务** - 点击「新建任务」按钮
3. **配置任务**：
...
**内容生成**：
- **使用 AI**：是否使用 AI 生成回复内容
- **回复风格**：KOC 风格 / 专业风格 / 幽默风格
- **模板列表**：不使用 AI 时的回复模板

4. **保存任务** - 可以手动执行或设置定时执行

#### 任务执行

- **手动执行**：点击任务卡片上的「执行」按钮
- **暂停/恢复**：执行中可以暂停或恢复任务
- **查看历史**：在「抖音统计」页面查看回复历史

### 📊 抖音数据统计（本分支特色）

查看和分析抖音评论任务的执行情况：

1. **进入统计页面** - 点击左侧导航栏的「抖音统计」图标
2. **查看数据**：
   - 回复总数统计
   - 按任务分类的回复记录
   - 回复内容和时间详情
...
```bash
# 1. 克隆仓库（本分支）
git clone https://github.com/alin132/AutoGLM-GUI---Scheduled_tasks.git
cd AutoGLM-GUI---Scheduled_tasks
