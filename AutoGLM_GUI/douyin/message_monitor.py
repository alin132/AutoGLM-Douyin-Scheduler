...
class DouyinMessageMonitor:
    ...
    def __init__(self):
        ...
        self._config_path = Path.home() / ".config" / "autoglm" / "douyin_monitor.json"
        self._history_path = Path.home() / ".config" / "autoglm" / "douyin_reply_history.json"
        ...
        self._fastgpt_api_key: str = ""
        self._fastgpt_timeout: int = 60
    ...
    def _load_config(self) -> None:
        ...
        try:
            with open(self._config_path, encoding="utf-8") as f:
                ...
                self._fastgpt_api_key = config.get("fastgpt_api_key", "")
                self._fastgpt_timeout = config.get("fastgpt_timeout", 60)
    ...
    def update_config(
        ...
        if fastgpt_base_url is not None:
            self._fastgpt_base_url = fastgpt_base_url
        if fastgpt_api_key is not None:
            self._fastgpt_api_key = fastgpt_api_key
        if fastgpt_timeout is not None:
            self._fastgpt_timeout = max(10, fastgpt_timeout)
        self._save_config()
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if choices:
                    ...
                    if reply:
                        self._add_log(f"[FastGPT] 回复成功")
                        return reply
            
            self._add_log(f"[FastGPT] 响应异常: {resp.status_code}", "error")
            return None
            
        except requests.Timeout:
            self._add_log("[FastGPT] 请求超时", "error")
            return None
        except Exception as e:
            self._add_log(f"[FastGPT] 错误: {e}", "error")
            return None

    def _generate_reply(self, message: str, sender_name: str = "用户") -> str:
        """根据消息内容生成回复，优先使用 FastGPT."""
        ...
        if self._fastgpt_enabled:
            ...
            self._add_log("FastGPT 未返回有效回复，使用关键词匹配")
